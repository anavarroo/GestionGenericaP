...
class UserControllerTest {
    ...
    @Test
    void crearUsuario() {
        User user = new User(1, "Alejandro", "Ramirez", 30, "ale@example.com", "123 street","+34 123456788", true, "password");
    ...
    void consultarUsuarioPorTelefono() {
        ...
        List<UserDto> userDtoList = Collections.singletonList(new UserDto());
        when(userService.consultarUsuarioPorTelefono(String.valueOf(telefono))).thenReturn(userDtoList);

        ResponseEntity<List<UserDto>> response = userController.consultarUsuarioPorTelefono(String.valueOf(telefono));

        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(userDtoList, response.getBody());
        verify(userService, times(1)).consultarUsuarioPorTelefono(String.valueOf(telefono));
    }
